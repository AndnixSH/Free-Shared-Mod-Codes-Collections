...
public class TerraTechModMenu : MonoBehaviour
{
    ...
    // Damage multiplier
    public static float speedHack = 1;
    ...
    public static void MyMenu()
    {
        ...
        if (GUI.Button(BtnRect(3, true), "Speed hack (-/+): x" + speedHack.ToString(), BtnStyle))
        {

        }
        if (GUI.Button(new Rect(posRect.x + widthSize - 35, posRect.y + btnY, 40, 40), "-", OffStyle))
        {
            if (speedHack > 1 && speedHack <= 30)
                speedHack--;
        }
        ...
        // Button click
        if (GUI.Button(BtnRect(4, false), "Visit sbenny.com", BtnStyle))
    ...
    public static void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.KeypadPlus))
        {
            ...
            ShowNotifi = true;
            delay = 5;
            nameOfStr = "Damage multiplier: " + speedHack.ToString();
            lastInput = string.Empty;
        }
