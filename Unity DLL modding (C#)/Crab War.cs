...
public class CrabWar : MonoBehaviour
{
    /// fields
    ...
    public static float delay = 0, widthSize = 400; // Change whole width size of menu here
    ...
    public static void ModMenuGUI()
    {
        ...
        GUI.Label(new Rect(rect.x + 55f, rect.y + 75f, widthSize + 10, 95f), "AndnixSH - platinmods.com xx", LabelStyle);
        ...
        //       return this.get_dmg;
        // }
		if (GUI.Button(BtnRect(1, false), "Add chests + event chests", BtnStyle))
        {
            MonoSingleton<CoopManager>.Instance.AddChest(1);
			MonoSingleton<CoopManager>.Instance.AddEventChest(1);
        }
		
		if (GUI.Button(BtnRect(2, false), "Complete Idol Quest", BtnStyle))
        {
           MonoSingleton<IdolQuestManager>.Instance.CompleteIdolQuest(1);
        }
		if (GUI.Button(BtnRect(3, false), "Give me, crystal, dna, gem, skills", BtnStyle))
        {
           MonoSingleton<CrystalManager>.Instance.AcquireCrystal(MonoSingleton<CrystalManager>.Instance.GetRandomNewCrystal());
		   PlayerSystem.Instance.AddRelics(BigNumber.Max(1, PlayerSystem.Instance.DisplayedRelics));
	PlayerSystem.Instance.SyncDisplayedRelics();
	PlayerSystem.Instance.AddGems(Mathf.Max(1, PlayerSystem.Instance.DisplayedGems));
	PlayerSystem.Instance.SyncDisplayedGems();
	MonoSingleton<SkillManager>.Instance.AddSkillPoints(50);
	MonoSingleton<StatisticsManager>.Instance.UpdateStatistic(StatisticsManager.AllTimeStatisticsType.AccumulatedSkillPointsCollected, 50.0);
	MonoSingleton<MonthlySkillManager>.Instance.AddSkillPoints(5);
	
        }
		if (GUI.Button(BtnRect(4, false), "Increase Progress", BtnStyle))
        {
           PlayerSystem.Instance.IncreaseProgress(9);
	MonoSingleton<UIManager>.Instance.UpdateProgress(PlayerSystem.Instance.Progress);
	MonsterSystem.Instance.ActiveMonster.Kill();
        }
		if (GUI.Button(BtnRect(5, false), "Kill Monster", BtnStyle))
        {
           MonsterSystem.Instance.ActiveMonster.Kill();
        }
		if (GUI.Button(BtnRect(6, false), "Learn All Skills", BtnStyle))
        {
           for (int i = 0; i < MonoSingleton<SkillManager>.Instance.skillList.Count; i++)
	{
		SkillManager.Skill skill = MonoSingleton<SkillManager>.Instance.skillList[i];
		if (!skill.learned)
		{
			if (MonoSingleton<SkillManager>.Instance.SkillPoints < skill.settings.cost)
			{
				MonoSingleton<SkillManager>.Instance.AddSkillPoints(skill.settings.cost);
			}
			if (MonoSingleton<SkillManager>.Instance.LearnSkill(skill))
			{
			}
		}
	}
	for (int j = 0; j < MonoSingleton<SkillTreeManager>.Instance.skillNodes.Count; j++)
	{
		MonoSingleton<SkillTreeManager>.Instance.skillNodes[j].UpdateNode();
	}
        }
		if (GUI.Button(BtnRect(7, false), "Reset All Cooldown", BtnStyle))
        {
           for (int i = 0; i < MonoSingleton<PowerUpManager>.Instance.powerUps.Length; i++)
	{
		MonoSingleton<PowerUpManager>.Instance.powerUps[i].ResetCooldown();
	}
        }
		if (GUI.Button(BtnRect(8, false), "Unlock all coop difficulties", BtnStyle))
        {
           MonoSingleton<StatisticsManager>.Instance.UpdateStatistic(StatisticsManager.AllTimeStatisticsType.HighestCoopLevelDefeated, 9.0);
	MonoSingleton<StatisticsManager>.Instance.UpdateStatistic(StatisticsManager.AllTimeStatisticsType.HighestCoopLevelUnlocked, 10.0);
        }
		
        /// Clicker Button
        // Open URL
        if (GUI.Button(BtnRect(9, false), "Visit PMT", BtnStyle))
        {
