...
public class OFDP : MonoBehaviour {

	public static bool m_bAutoPlayMode = true;

	private static float m_autoPlayTime;

	public static float m_autoPlayResponseTime;

	public static float m_randomResponseGap;

	private static float m_fRandomGap;

	public static bool m_bFullHP;
    ...
    public static void ModMenuGUI()
    {
        ...
        if (toggle1)
        {
            if (GUI.Button(BtnRect(1, false), "No miss delay: ON", OnStyle))
            {
                toggle1 = false;
            }
        }
        else if (GUI.Button(BtnRect(1, false), "No miss delay: OFF", OffStyle))
        {
            toggle1 = true;
        }
        ...
        if (toggle2)
        {
            if (GUI.Button(BtnRect(2, false), "Auto play: ON", OnStyle))
            {
                toggle2 = false;
            }
        }
        else if (GUI.Button(BtnRect(2, false), "Auto play: OFF", OffStyle))
        {
            toggle2 = true;
        }
        ...
        if (toggle3)
        {
            if (GUI.Button(BtnRect(3, false), "God mode: ON", OnStyle))
            {
                toggle3 = false;
            }
        }
        else if (GUI.Button(BtnRect(3, false), "God mode: OFF", OffStyle))
        {
            toggle3 = true;
        }


        if (GUI.Button(BtnRect(4, false), "Visit platinmods.com", BtnStyle))
        {
	...
	{
		if (toggle3)
			GameManager.Instance.m_Hero.Life = 99999999f;
		if (toggle2)
		{
			OFDP.DoAutoPlay();
		}
		//if (OFDP.m_bFullHP && GameManager.Instance.m_Hero)
		//{
	...
	private static void DoAutoPlay()
	{
		...
		if (OFDP.m_autoPlayTime + OFDP.m_autoPlayResponseTime + OFDP.m_fRandomGap < Time.unscaledTime)
		{
			OFDP.m_autoPlayTime = Time.unscaledTime;
			OFDP.m_fRandomGap = UnityEngine.Random.Range(0f, OFDP.m_randomResponseGap);
			...
			else
			{
				...
				if (flag)
				{
					flag = OFDP.IsNotAngel(GameManager.Instance.GetTagetEnemy(Unit.eDirection.Left, true));
				}
				if (flag2)
				{
					flag2 = OFDP.IsNotAngel(GameManager.Instance.GetTagetEnemy(Unit.eDirection.Right, true));
				}
				if (flag && flag2)
				{
	...
	private static bool IsNotAngel(Unit unit)
	{
